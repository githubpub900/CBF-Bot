...
namespace bot {
    ...
    // RobTop's "Click Between Steps" is a GD game-variable. Unlike a Geode
    // mod it is not advertised through the loader, so we have to read it
    // from the GameManager. Verified against CBF's own source
    // (theyareonit/Click-Between-Frames): the ID is "0177" for CBS and
    // "0176" for Click ON Steps (a coarser variant). If a future GD update
    // moves them, these are the only lines that need touching.
    static constexpr const char* ROBTOP_CBS_GAMEVAR = "0177";
    static constexpr const char* ROBTOP_COS_GAMEVAR = "0176";
    ...
    // v6: (time double, button, flags)
    // v7: (tick i64, time double, button, flags) -- primary key is tick, an
    //     integer substep counter we own, so playback firing is immune to
    //     any float drift between the recording and replay clocks.
    static constexpr uint32_t MACRO_VERSION = 7;
...
//
struct InputEvent {
    // Both a substep tick (integer, our own counter -- the primary key for
    // playback firing) and a level-time double (informational + fallback).
    // Recording writes BOTH; playback compares tick <= simTick, immune to
    // any float drift between recording and playback clocks.
    double    time    = 0.0;   // m_gameState.m_levelTime when the input fired
    long long tick    = 0;     // BotManager::simTick when the input fired
    uint8_t   button  = 1;
    bool      down    = true;
    bool      player2 = false;

    InputEvent() = default;
    InputEvent(double t, long long tk, uint8_t b, bool d, bool p2)
        : time(t), tick(tk), button(b), down(d), player2(p2) {}

    bool operator<(InputEvent const& o) const {
        // Time is the authoritative key (playback fires on level-time
        // windows). Tick is only a coarse file-format shadow of it.
...
public:
    ...
    // ----- substep detection -----------------------------------------------
    //
    // Playback and recording are keyed on the RAW level-time (see
    // recordInput / fireDueInputs), which is deterministic for a given level
    // position and identical regardless of FPS, CBF subdivision, or the
    // speedhack multiplier. `simTick` is NOT the timing key anymore -- it is
    // only a monotonic counter used to detect "a genuine new substep has
    // begun" (via advanceSimTick) so the per-substep hooks fire exactly once
    // even though PlayerObject::update is called for several PlayerObjects
    // (icon-kit previews, ghost trails, dual players) within one substep.
    //
    // TICKS_PER_SEC = 240 mirrors CBF's own substep rate (delta*60*4) and is
    // still written into the file format as a coarse shadow of the time for
    // backward/forward compatibility.
    static constexpr int TICKS_PER_SEC = 240;
    long long   simTick        = 0;
    double      lastSeenTime   = -1.0;

    static long long tickFromTime(double t) {
        if (!std::isfinite(t) || t < 0.0) return 0;
        return static_cast<long long>(std::floor(t * TICKS_PER_SEC + 0.5));
    }
    double timeFromTick(long long tk) const {
        return static_cast<double>(tk) / static_cast<double>(TICKS_PER_SEC);
    }
    void resyncSimTickTo(double t) {
        simTick = tickFromTime(t);
        lastSeenTime = t;
    }
    // Called from onSubStepBegin. Detects a genuine new substep by clock
    // change and bumps simTick. Any hook call with the same clock value
    // (extra PlayerObjects, reentrancy) is a NO-OP.
    bool advanceSimTick(GJBaseGameLayer* gl) {
        double now = levelTime(gl);
        if (!std::isfinite(now)) return false;
        if (now < lastSeenTime - 0.001) {
            // Clock jumped backwards (checkpoint / restart) -- resnap.
            resyncSimTickTo(now);
            return true;
        }
        if (now > lastSeenTime + 1e-9) {
            simTick = tickFromTime(now);
            lastSeenTime = now;
            return true;
        }
        return false;
    ...
    bool isRobTopCBSEnabled() const {
        ...
        // Either CBS ("0177") or Click ON Steps ("0176") counts as active
        // for our precision needs -- both give ~480Hz sub-frame input.
        auto gm = GameManager::sharedState();
        if (!gm) return false;
        return gm->getGameVariable(bot::ROBTOP_CBS_GAMEVAR) ||
               gm->getGameVariable(bot::ROBTOP_COS_GAMEVAR);
    }
    ...
    void startRecording(GJBaseGameLayer* gl) {
        ...
        resetHeldState();
        resyncSimTickTo(macro.recordStartTime);
        autoClickLastEdge = 0;
    ...
    // Move the playback cursor to the first event at or after `time` (the
    // first event that hasn't been "consumed" yet at this level position).
        ...
        for (int pi = 0; pi < 2; ++pi) {
            for (int b = 1; b <= 3; ++b) {
                if (held[pi][b]) {
                    ...
                    long long lastTick =
                        macro.events.empty() ? 0 : macro.events.back().tick;
                    macro.events.emplace_back(
                        lastTime + 0.001, lastTick + 1,
    ...
    // for the same button/player) so the macro stays clean.
       void recordInput(GJBaseGameLayer* gl, bool down, int button, bool isPlayer1) {
        ...
        // Stamp with the RAW level-time (m_levelTime), which CBF advances to
        // this substep's exact value before dispatching the queued input via
        // handleButton. Playback fires an event on the substep whose window
        // [now, now+dt) contains this time (see fireDueInputs), so the input
        // lands on exactly the substep it was recorded on -- no 240 Hz
        // rounding, and speed-independent because level-time is the same for
        // a given level position regardless of the speedhack multiplier.
        //
        // Keep events strictly increasing in time so the playback cursor is
        // unambiguous: two inputs in the same substep get nudged apart by a
        // single ULP (still well within one substep window).
        double t = levelTime(gl);
        if (!macro.events.empty() && t <= macro.events.back().time) {
            t = std::nextafter(macro.events.back().time,
                               std::numeric_limits<double>::infinity());
        }
        long long tk = tickFromTime(t); // kept for file-format compatibility

        InputEvent e(t, tk, static_cast<uint8_t>(button), down, player2);
    ...
    // Single authoritative "is this player's gravity flipped" check, used by
    // BOTH the live-input path and the per-substep correction so they can
    // never disagree (a disagreement produced a one-off wrong-direction
    // glitch when the user pressed/released within a substep of a flip).
    // m_isUpsideDown is GD's gravity-flip flag; the flipGravity hook makes
    // it fresh at the exact moment a portal toggles it.
    static bool waveUpside(PlayerObject* p) {
        return p && p->m_isUpsideDown;
    }
    // Is this player currently a wave/dart?
    static bool waveIsDart(PlayerObject* p) {
        return p && p->m_isDart;
    }
    ...
    bool filterLiveJump(bool& down, bool isPlayer1) {
        ...
        PlayerObject* p = pl ? (isPlayer1 ? pl->m_player1 : pl->m_player2)
                             : nullptr;
        if (!waveIsDart(p)) { waveEffectiveHeld[pi] = down; return true; }
        // effective = raw XOR upside: holding through a flip becomes a
        // release, releasing after a flip becomes a hold -- so the wave
        // keeps travelling the direction the player is steering as if the
        // gravity change never happened.
        bool effective = (down != waveUpside(p));
    ...
    void tickWaveGravityFor(GJBaseGameLayer* gl, PlayerObject* p, bool isP1) {
        ...
        // Same XOR as filterLiveJump, same upside check. Outside the wave
        // the correction is a pass-through of the raw hold (also releases a
        // phantom hold the instant a portal takes the player out of wave).
        bool upside = waveUpside(p);
        waveLastUpsideDown[pi] = upside;

        bool effective = waveIsDart(p) ? (waveRawHeld[pi] != upside)
                                        : waveRawHeld[pi];
        ...
        if (!effective) {
            // Full jump-buffer teardown -- any one of these left set will
            // silently re-press the jump on the next substep and override
            // the release we just issued.
    ...
    void onSubStepBegin(GJBaseGameLayer* gl, PlayerObject* p, bool isP1, float dt) {
        if (inSubStep) return;
        inSubStep = true;
        double now = levelTime(gl);
        tickWaveGravityFor(gl, p, isP1);
        // Detect a NEW substep by m_levelTime change (not just any hook
        // call -- extras from other PlayerObjects share the same
        // m_levelTime and must be no-ops).
        bool newSubstep = advanceSimTick(gl);
        if (newSubstep && isP1) {
            if (mode == bot::Mode::Playing)
                fireDueInputs(gl, now, static_cast<double>(dt));
    ...
    //
    // Called at the START of a substep, before its physics runs. `now` is
    // the current level-time (start of this substep) and `dt` is the
    // substep's delta, so the substep about to run covers [now, now + dt).
    // We fire every recorded event whose time falls inside that window --
    // i.e. every input that, during recording, was dispatched during this
    // same substep. That reproduces the run substep-for-substep: an input
    // recorded at level-time T is re-applied before the exact substep whose
    // window contains T, at any FPS, any speedhack multiplier, and any CBF
    // subdivision (all of which leave level-time unchanged for a given
    // level position). No 240 Hz quantization, so hold lengths are exact.
    void fireDueInputs(GJBaseGameLayer* gl, double now, double dt) {
        if (mode != bot::Mode::Playing) return;
        if (!gl) return;

        // Guard a bad/first-frame dt so the horizon is always ~one substep.
        if (!(dt > 0.0) || dt > 0.1) dt = 1.0 / 240.0;
        double horizon = now + dt;

        injecting = true;
        while (playbackIndex < macro.events.size() &&
               macro.events[playbackIndex].time < horizon) {
    ...
    // physics snapshot to fix the practice bug, and re-align playback.
      void onCheckpointLoaded(PlayLayer* pl, void* cpPtr) {
        ...
        // Re-anchor the sim tick and autoclick edge to the resume time.
        resyncSimTickTo(frame->levelTime);
        autoClickLastEdge = 0;
    ...
    // True only while a player is alive in a level. Used to gate BOTH the
    // physics time-warp and the audio pitch: during the death / respawn
    // window we hand both back to 1.0 so the music does not run at the
    // pitched rate while the level clock is paused (which drifted the song
    // out of sync with the level), and so the respawn delay itself is not
    // stretched by the speedhack.
    ...
    void applyMusicSpeed() {
        ...
        float pitch = (speedhackEnabled && playerAlive() &&
    ...
    bool saveMacro(std::string const& name) {
        ...
        for (auto const& e : macro.events) {
            ...
            wr(&e.tick, 8);       // int64, our substep counter (primary key)
            wr(&e.time, 8);       // double, level-time seconds (secondary)
            wr(&e.button, 1);
            wr(&flags, 1);
        }
        out.close();
    ...
    bool loadMacro(std::string const& name) {
        ...
        for (uint32_t i = 0; i < count && in; ++i) {
            ...
            uint8_t flags = 0;
            if (version >= 7) {
                // v7+: substep tick (int64) + level-time (double).
                rd(&e.tick, 8);
                rd(&e.time, 8);
            } else if (version >= 6) {
                // v6: level-time based (double). Derive tick from time.
                rd(&e.time, 8);
                e.tick = tickFromTime(e.time);
            } else if (version == 5) {
                // v5: X-position based (float). Unrecoverable.
                float oldX = 0.f;
                rd(&oldX, 4);
                e.time = 0.0;
                e.tick = 0;
            } else {
                // v4 and below: time-based (double). Derive tick.
                rd(&e.time, 8);
                e.tick = tickFromTime(e.time);
            }
            rd(&e.button, 1);
    ...
    std::filesystem::path physicsMacroPath(std::string const& name) const {
       ...
       bool savePhysicsMacro(std::string const& name) {
        ...
        uint32_t version = 6;  // v6: tick primary + time secondary
        ...
        // Write input events (tick primary + time secondary).
        for (auto const& e : macro.events) {
            uint8_t flags = (e.down ? 1 : 0) | (e.player2 ? 2 : 0);
            wr(&e.tick, 8);
            wr(&e.time, 8);
...
protected:
    ...
    cocos2d::CCLayerColor*  m_periodLabel = nullptr; // coloured status swatch
    ...
    // Panel geometry. Wider + taller so nothing overlaps at bigFont scales,
    // with kPanelScale=0.75 to still fit inside GD's ~569x320 design res.
    static constexpr float kPanelW     = 600.f;
    static constexpr float kPanelH     = 360.f;
    static constexpr float kPanelScale = 0.75f;
...
private:
    ...
    void buildPanel() {
        ...
        // Settings gear in the top-left corner. Bigger than before.
        cocos2d::CCNode* gearSpr =
            CCSprite::createWithSpriteFrameName("GJ_optionsBtn_001.png");
        if (gearSpr) {
            gearSpr->setScale(0.55f);
        } else {
        ...
        xBtn->setPosition({ W - 28.f, H - 28.f });
        menu->addChild(xBtn);

        auto& bot = BotManager::get();

        // ---- LEFT COLUMN: mode / options / speed / info ------------------
        // Vertical rhythm is 44px per row so nothing collides.
        constexpr float xL = 48.f;
        float y = H - 70.f;
        m_recordToggle = makeToggle(menu, m_panel, { xL, y },
            menu_selector(BotUILayer::onRecord), "Record (V)",
            bot.mode == bot::Mode::Recording);
        m_playToggle = makeToggle(menu, m_panel, { xL + 150.f, y },
        ...
        m_deadToggle = makeToggle(menu, m_panel, { xL + 150.f, y },
        ...
        // Info triple: mode / progress / stats. Vertically stacked with
        // enough gap they never touch the row above.
        y -= 42.f;
        m_modeLabel = CCLabelBMFont::create("Mode: idle", "bigFont.fnt");
        m_modeLabel->setAnchorPoint({ 0.f, 0.5f });
        m_modeLabel->setPosition({ xL - 18.f, y });
        m_modeLabel->setScale(0.42f);
        ...
        m_progressLabel->setPosition({ xL - 18.f, y });
        m_progressLabel->setScale(0.38f);
        ...
        m_statsLabel->setPosition({ xL - 18.f, y });
        m_statsLabel->setScale(0.34f);
        m_statsLabel->setOpacity(190);
        m_panel->addChild(m_statsLabel);

        // CBF STATUS: bottom-left. The indicator is a small SQUARE swatch
        // (not a "." glyph, whose ink sits low in the character cell and
        // never lined up with the text). A CCLayerColor lets us place its
        // exact centre on the text's centre Y so the two read as one line.
        constexpr float cbfY = 20.f;
        constexpr float swatch = 12.f;
        m_periodLabel = cocos2d::CCLayerColor::create({0, 255, 0, 255},
                                                      swatch, swatch);
        m_periodLabel->setAnchorPoint({ 0.f, 0.5f });
        // CCLayerColor's position is its bottom-left; offset down by half
        // its height so its centre lands on cbfY.
        m_periodLabel->setPosition({ xL - 18.f, cbfY - swatch * 0.5f });
        m_panel->addChild(m_periodLabel);

        m_statusLabel = CCLabelBMFont::create("CBF: ...", "bigFont.fnt");
        m_statusLabel->setAnchorPoint({ 0.f, 0.5f });
        m_statusLabel->setPosition({ xL - 18.f + swatch + 6.f, cbfY });
        m_statusLabel->setScale(0.36f);
        m_panel->addChild(m_statusLabel);

        // ---- RIGHT COLUMN: macro list + info + name + save/load ----------
        // Everything below is laid out from the top down and MUST land above
        // y=0. listH is sized so the last button (Clear) clears the bottom.
        constexpr float xR = 320.f;
        constexpr float listW = 240.f, listH = 150.f;
        auto listTitle = CCLabelBMFont::create("Macros", "bigFont.fnt");
        listTitle->setAnchorPoint({ 0.5f, 0.5f });
        listTitle->setPosition({ xR + listW * 0.5f, H - 40.f });
        listTitle->setScale(0.55f);
        m_panel->addChild(listTitle);

        float listTopY = H - 56.f;
        m_listBg = cocos2d::extension::CCScale9Sprite::create("GJ_square02.png");
        m_listBg->setContentSize({ listW, listH });
        m_listBg->setPosition({ xR + listW * 0.5f, listTopY - listH * 0.5f });
        m_panel->addChild(m_listBg);

        m_macroList = geode::ScrollLayer::create({ listW - 8.f, listH - 8.f });
        m_macroList->setPosition({ xR + 4.f, listTopY - listH + 4.f });
        ...
        // Info line under the list.
        float infoY = listTopY - listH - 15.f;   // ~ 139
        ...
        // Name box on its own row.
        float nameY = infoY - 30.f;               // ~ 109
        ...
        // Save / Load / Del / Refresh, evenly spaced.
        float btnY = nameY - 34.f;                 // ~ 75
        ...
        // "Clear current" -- clears the loaded macro in memory (distinct
        // from Del which removes a saved file). Sits fully inside the panel.
        makeButton(menu, { xR + listW * 0.5f, btnY - 30.f }, "Clear current",
                   menu_selector(BotUILayer::onClear));  // ~ 45

        // Keybind hint: bottom-RIGHT corner, clear of the CBF strip (bottom
        // left) and the Clear button (which is higher up, centred on xR).
        auto hint = CCLabelBMFont::create(
            "K menu   V rec   B play   N stop   M step",
            "bigFont.fnt");
        hint->setAnchorPoint({ 1.f, 0.5f });
        hint->setPosition({ W - 20.f, 16.f });
        hint->setScale(0.3f);
        hint->setOpacity(150);
   ...
   void onSpeedToggle(CCObject*) {
    ...
    void updateSelectionInfo() {
        ...
        double duration = 0.0;
        if (count > 0) {
            // v7+: 8(tick)+8(time)+1(button)+1(flags) = 18 bytes/event.
            // Older: 8(time)+1(button)+1(flags) = 10 bytes/event.
            std::streamoff perEvent = (version >= 7) ? 18 : 10;
            std::streamoff timeOffset = (version >= 7) ? 8 : 0;
            in.seekg(-perEvent + timeOffset, std::ios::end);
