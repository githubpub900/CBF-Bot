...
class $modify(BotBaseGameLayer, GJBaseGameLayer) {
    ...
    void update(float dt) {
        ...
        // here makes the speedhack scale everything (m_levelTime, substeps,
        // music) uniformly and by the same code path GD's own speedhack
        // would use.
        //
        // ONLY warp while the player is ALIVE. During the death / respawn
        // window the level clock is paused but the song keeps playing --
        // warping there stretched the respawn delay and drifted the music
        // out of sync. Hand time back to 1.0 when dead (or when the hack is
        // off) so respawn timing is normal and the song stays aligned.
        if (isPlay(this)) {
            bool alive = BotManager::playerAlive();
            float target = (bot.speedhackEnabled && alive)
...
class $modify(BotPlayerObject, PlayerObject) {
    ...
    void update(float dt) {
        ...
        auto& bot = BotManager::get();
        bot.onSubStepBegin(pl, this, isP1, dt);
        PlayerObject::update(dt);
