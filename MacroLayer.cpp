...
MacroLayer* MacroLayer::create() {
    ...
    if (ret && ret->initAnchored(380.f, 320.f, "GJ_square01.png")) {
        ret->autorelease();
        return ret;
    }
    CC_SAFE_DELETE(ret);
...
void MacroLayer::toggle() {
    ...
    if (!scene) return;
    // If a MacroLayer is already up, close it; otherwise open one.
...
bool MacroLayer::setup() {
    ...
    actionMenu->setPosition({cx, winSize.height - 130.f});
    ...
    m_filenameInput->setPosition({cx - 50.f, winSize.height - 180.f});
    ...
    fileMenu->setPosition({cx + 90.f, winSize.height - 175.f});
    ...
    m_speedSlider->setPosition({cx, winSize.height - 235.f});
...
void MacroLayer::refreshUI(float /*dt*/) {
    ...
    bool cbf = CBFIntegration::get().isCBFActive();
    m_cbfLabel->setString(fmt::format(
        "CBF: {} | tps: {:.0f}",
        cbf ? "ACTIVE (sub-frame)" : "inactive (per-frame)",
        CBFIntegration::get().getTicksPerSecond()
    ).c_str());
...
void MacroLayer::onSpeedSlider(CCObject*) {
    // Slider value is 0..1; map to 0.1x .. 10x logarithmically.
    float t = m_speedSlider->getValue();
    // Map [0,1] -> [0.1, 10] via log scale: speed = 0.1 * 100^t
    float speed = 0.1f * std::pow(100.0f, t);
    MacroBot::get().setSpeed(speed);
    m_speedLabel->setString(fmt::format("Speed: {:.2f}x", speed).c_str());
}
